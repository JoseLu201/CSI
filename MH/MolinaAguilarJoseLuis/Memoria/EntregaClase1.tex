...
\section{Práctica2a : Técnicas de Búsqueda Basados en Poblaciones}
...
Cabe destacar que el algoritmo de BL usado es el mismo que el de la practica 1 excepto que en vez de generar una solucion se la pasamos como parametro,
...
\begin{figure}[h]
  \centering
  \includegraphics[width=\textwidth]{ag_compare}
...
\begin{figure}[h]
  \centering
  \includegraphics[width=\textwidth]{age_comp}
...
\begin{figure}[h]
  \centering
  \includegraphics[width=\textwidth]{am_compare}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{tiempos_compa}
  \caption{Desviación distintos Algoritmos}
  \label{fig:tiem}
\end{figure}
     ...
     \begin{algorithm}[H]
       \begin{algorithmic}[1]
         \Function{$EnfriamientoSimulado(MAX_ITERS, fit_solucion)$}{}
          ...
          \EndWhile
          \State $ sol\_fit \gets best\_fit$   \Comment{Lo devuelvo por referencia}       
          \State \Return{$best$}
         \EndFunction
       \end{algorithmic}
     \end{algorithm}

     Pseudocodigo de \textbf{BMB(n\_sol, MAX\_ITERS)}
     \begin{algorithm}[H]
       \begin{algorithmic}[1]
         \Function{$BMB(n\_sol, MAX\_ITERS)$}{}
          \State $solucion \gets \emptyset$
          \State $best \gets \emptyset$
     ...
     \begin{algorithm}[H]
       \begin{algorithmic}[1]
         \Function{$ILS(n\_sol, ILS\_ITERS)$}{}
          ...
          \While{$iters < n\_sol$}
            \State $ iters \gets iters + 1$
            \State $BL\_sol \gets BL\_2(solucion,BL\_ITERS,curr\_fit)$ \Comment{devuelve el fit por referencia}
