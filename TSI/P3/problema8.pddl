(define (problem ejercicio7) 
    ...
    (:init
        ...
        ; Añadimos las restricciones de tiempo
        ; Iniciamos el tiempo a 0
        (= (tiempo) 0)

        ; Asignamos el tiempo de recoleccion a 5 ud de tiempo.
        (= (tRecolectar) 5)

        ; La distancia entre localizaciones es de 20 unidades
        (= (distancia) 20)

        ; La velocidad de cada unidad
        (= (velocidad VCE) 1)
        (= (velocidad Marine) 5)
        (= (velocidad Soldado) 10)

        ; Tiempos de creacion de Unidades/Edicios
        (= (tUnidad VCE) 10)
        (= (tUnidad Marine) 20)
        (= (tUnidad Soldado) 30)

        (= (tConstr Barracones) 50)
        (= (tConstr Extractor) 20)
