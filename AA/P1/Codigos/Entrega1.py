...
#----------------IMPORTANTE------------------#
#Para ver graficas auxiliares cambiar a True !!!
...
#--------------------------
'''
Funcion que dada unas conjunto de datos y etiquetas para diferenciar los datos
los dibuja junto con una recta de regresion
'''
...
#Dibuja la funcion junto al recorrido hacia el optimo
...
#input("\n--- Pulsar tecla para continuar con el ejercicio 1.3 ---\n")
...
print ('Numero de iteraciones: ', it_1)
print ('Coordenadas obtenidas: (', w[0], ', ', w[1],')')
print()

display_figure(3, f1, ws_1, 'plasma','3a, eta = 0.01',r'x^2+ 2y^2 +2sin(2\pi x)sin(\pi y)')
...
print ('Numero de iteraciones: ', it_2)
print ('Coordenadas obtenidas: (', w[0], ', ', w[1],')')

#Esta parte dibuja el grafico de iteraciones / tasa de aprendizaje de los dos casos anteriores
if(secundary_things == True):
    ws_1 = np.asarray(ws_1)
    ws_2 = np.asarray(ws_2)
    fig = plt.figure()
    ax_gra3= fig.add_subplot()
    ax_gra3= plt.plot(np.arange(it_1+1),f1(ws_1[:,0],ws_1[:,1]),'bo-')
    ax_gra3 = plt.plot(np.arange(it_2+1),f1(ws_2[:,0],ws_2[:,1]),'yo-.')
    ax_gra3 = plt.xlabel('Iteraciones')
    ax_gra3 = plt.ylabel('Valor de $f1$')
    plt.legend(['$\eta = 0.01$','$\eta = 0.1$'])

display_figure(3, f1, ws_2, 'plasma','3a, eta = 0.1',r'x^2+ 2y^2 +2sin(2\pi x)sin(\pi y)')
...
#           batch_size -> tamaño de la muestra sobre la que se va a ejecutar
#           maxIter -> numero max iteraciones
#           w -> vector de caracteristicas
#           La idea es crea un vector de indices aleatorios del tamaño de la muestra total
#           El mini_batch seran
#  
...
def sgd(x,y,eta,batch_size,maxIter,w,error2get):
    ...
    while(Err(x, y, w) > error2get and iterations < maxIter):
        ...
        #Aqui tenemos los indices a escoger de tamaño batch_size y sin repetir en 
        #proximas iteraciones
        mini_batch = indices[min_batch_n : max_iter_batch]
        
        #Calculo y actualizo los pesos
...
def pseudoinverse(x,y):
    ...
    #Tambien se puede hacer cn np.linalg.pinv(x)
...
def caract_vec(x1,x2):
    caract_vec = np.zeros((x1.size,6))
    caract_vec[:,0] = 1
    caract_vec[:,1] = x1
    caract_vec[:,2] = x2
    caract_vec[:,3] = x1*x2
    caract_vec[:,4] = np.power(x1,2)
    caract_vec[:,5] = np.power(x2,2)
    return caract_vec

#-----------------------------------------------------------
#Repetir el esperimento pero con carcteristicas no lineales
#En este caso tanto en x como en y ya no son los indices como antes, 
#ahora si son los valores concretos, pero sigue siendo la misma forma que antes
def gradient2(x,y,w):
...
data_set= np.insert(data_set,0,np.ones(len(data_set)),axis=1)
y_n = add_noise(y_n,0.1)
sample = caract_vec(data_set[:,1], data_set[:,2])
...
if secundary_things == True:
    ...
    maxIter = 20000
    error2get = 1e-9
    #Esto podria borrarlo
    #EN esta parte vemos como de importante es el valor del batch respecto del numero de iteraciones y el error
...
#Funcion para dibujar funciones no lineales
...
#------------------------------------------
#EN  parte dibujamos como se ajusta al data_set los pesos
w = np.zeros(6)
data_set = simula_unif(1000, 2, 1)
y_n = add_labels(data_set,f)
data_set= np.insert(data_set,0,np.ones(len(data_set)),axis=1)
y_n = add_noise(y_n,0.1)
x = caract_vec(data_set[:,1], data_set[:,2]) 
...
w,iterations_sgd,error = sgd2(x, y_n, eta, batch, maxIter, w, error2get)
non_linear_plot(data_set, y_n, w,title="Non Linear", xlabel="$x_1$", ylabel="$x_2$")

print ('Bondad del resultado el conjunto de datos en 1 iteracion para modelo no lineal:\n')
print ("Ein: ", Err(x,y_n,w))
print ("w : ", w)
print('Iteraciones: ' , iterations_sgd)
...
#Ultima parte de repetir el experimento para caracteristicas no lineales
...
for exp in range(1000):
    ...
    x = caract_vec(data_set_n[:,1], data_set_n[:,2]) 
    x_test = caract_vec(test_data_set_n[:,1], test_data_set_n[:,2])
    
    w_n,iterations_sgd,error = sgd2(x, y_n, eta, batch_size, maxIter, w, error2get)
    
    mean_err_in.append(Err(x,y_n,w_n))
    #print ("Ein: ",Err(x,y_n,w_n))
    #non_linear_plot(data_set_n, y_n, w_n,title="Non Linear", xlabel="r$x_1$", ylabel="r$x_2$")
    mean_err_out.append(Err(x_test, test_y_n, w_n))


mean_err_in = np.array(mean_err_in, np.float64)
mean_err_out = np.array(mean_err_out, np.float64)
print ('Bondad del resultado el conjunto de datos tras 1000 ejecuciones, no lineal:\n')
print ("Ein: ",mean_err_in.mean())
#print ("w : ", w_s)
print ("Eout: ",mean_err_out.mean())
print('Ha tardado ' , int(time.time() - start_time), ' segundos')
